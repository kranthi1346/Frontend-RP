...
export const JobDescriptionComponent: FC<JobDescriptionComponentProps> = ({
  setComponentState,
}) => {
    ...
    reader.onload = async (e) => {
      if (e.target?.result) {
        ...
        if (typeof e.target.result === "string") {
          result = await fetch(e.target.result).then((res) =>
            res.arrayBuffer()
          );
        } else {
  ...
  useEffect(() => {
    setComponentState("jobDescription", editorValue);
  }, [editorValue]);
